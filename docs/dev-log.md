...
- **제품 기획 Phase 1 진행(06-11~12)**: #1 상품 이미지 ✅`b830277`. #2 리뷰·평점(백엔드 `94b6304` + FE/수정 `441c12e`) ✅ push. #3 패션 필터/검색/정렬 UI + PLP 재설계 ✅ **커밋 `b71ded8`**. #4 체크아웃 완성 = **단계별(①수량 변경→②배송지/주소록→③주문서)**: **Step 1 수량 변경 ✅ 커밋 `4d4c41e`**, **Step 2 주소록(새 `address` 도메인·V16·FE `/account/addresses`) ✅ 정적+런타임 PASS·미커밋(187 tests)**. 다음 = #4 **Step 3 주문서**(`Order` 배송 스냅샷+V17·checkout 바디 확장·`/checkout` 확인 페이지) → Phase 1 마무리.
